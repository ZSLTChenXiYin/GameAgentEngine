...
var relationCreateCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if !containsString(sdk.ValidRelationTypes(), relationType) {
			fail(fmt.Errorf("invalid relation type %q; allowed: %s", relationType, validRelationTypesText()))
		}
...
var relationUpdateCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if typeChanged && !containsString(sdk.ValidRelationTypes(), relationType) {
			fail(fmt.Errorf("invalid relation type %q; allowed: %s", relationType, validRelationTypesText()))
		}
...
func init() {
	...
	relationListCmd.Flags().String("type", "", "按关系类型过滤（relation_type）")
	...
	relationCreateCmd.Flags().String("type", "", "关系类型："+validRelationTypesText())
	...
	relationUpdateCmd.Flags().String("type", "", "新的关系类型："+validRelationTypesText())
