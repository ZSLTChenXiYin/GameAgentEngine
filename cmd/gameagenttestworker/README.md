...
`GameAgentTestWorker` is a deterministic external test worker for GameAgentEngine full-functional validation.

It is intended for repeatable runtime-task and callback/resume scenarios, not for production game-side integration.
...
```powershell
GameAgentTestWorker.exe serve --verbose
```

Run only the push receiver:

```powershell
GameAgentTestWorker.exe push-receiver --push-port 9000
```

Run one pull step:

```powershell
GameAgentTestWorker.exe pull-once --consumer game_client
```
...
```powershell
GameAgentTestWorker.exe serve --fail-interface spawn_item
```

Simulate long-running execution with heartbeats:

```powershell
GameAgentTestWorker.exe serve --long-task-interface game_client_request_data --long-task-duration 8s --heartbeat-interval 2s
```
