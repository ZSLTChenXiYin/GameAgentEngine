...
GameAgentEngine v0.2.0 使用四种基本抽象来建模游戏世界：**节点（Node）**、**组件（Component）**、**记忆（Memory）** 和 **关系（Relation）**。它们共同构成一个有向图，表示每个实体、其属性、历史以及与其他实体的连接。引擎还提供推理管线、记忆传播系统和子任务 DAG 编排能力。
...
3. 策略引擎评估计划
4. 写入并传播记忆
5. 触发 world_tick_sync 自主节点
6. 创建 TimelineModel 记录
