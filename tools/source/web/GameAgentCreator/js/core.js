...
const i18n = {
  zh: {
    ...
    'Add tick units before configuring calendar values.': '请先添加 Tick 单位，再配置日历值。',
    ...
    'Parent': '父节点',
    'Primary Parent': '主父节点',
    'Primary parent updated': '主父节点已更新',
    'Primary Parent is the only hierarchy field shown in the outline. External Parents and other relations stay in the relations table.': 'Primary Parent 是大纲树里唯一展示的层级字段，External Parents 和其他关系会保留在 relations 表中。',
    'Primary Parent is the only hierarchy field shown in the outline. Use located_at for the current environment, belongs_to/subordinate for organization or control, and external_parent only for auxiliary DAG scope.': 'Primary Parent 是大纲树里唯一展示的层级字段。当前环境请使用 located_at，组织或控制链请使用 belongs_to/subordinate，external_parent 只用于辅助 DAG 范围。',
    ...
    'Publish Up': '继续向上发布',
    'Publish Up only extends environment_scope or organization_scope beyond their scoped graph. It has no effect on targeted or manual propagation.': 'Publish Up 只会让 environment_scope 或 organization_scope 在其作用域之外继续向上扩展，对 targeted 和 manual 不生效。',
    'Outgoing': '出向',
    ...
    'Relations do not change the outline hierarchy unless you separately edit Primary Parent.': '除非你另外修改 Primary Parent，否则关系本身不会改变大纲层级。',
    'This target is already the Primary Parent. If the node is only temporarily here, prefer keeping Primary Parent stable and using located_at for movement.': '这个目标已经是 Primary Parent。如果节点只是临时在这里，优先保持 Primary Parent 稳定，并用 located_at 表达移动。',
    'This node already has another located_at relation. Keep only one active current environment unless you intentionally model multiple simultaneous positions.': '这个节点已经有另一条 located_at 关系。除非你确实要表达同时处于多个位置，否则应只保留一个当前环境。',
    'This target is already the Primary Parent. If you want to change stable hierarchy, edit Primary Parent directly. Use this relation only to add organization/control semantics.': '这个目标已经是 Primary Parent。如果你要修改稳定层级，请直接编辑 Primary Parent。这个关系只应用来补充组织或控制语义。',
    'external_parent is auxiliary DAG scope only. It is excluded from default context assembly and default propagation, so do not rely on it for current location or primary organization modeling.': 'external_parent 仅用于辅助 DAG 范围。它不会进入默认上下文组装，也不会参与默认传播，因此不要用它表达当前位置或主要组织归属。',
    'This node already has another external_parent relation. Keep this relation rare and use it only when a second parent-like scope is truly required.': '这个节点已经有另一条 external_parent 关系。请尽量少用这类关系，只在确实需要第二条父向作用域时再使用。',
    ...
    'Follow the stable Primary Parent chain upward. This is the default mode for promoting local memory into broader identity context.': '沿稳定的 Primary Parent 链向上传播。这是把局部记忆提升到更大身份上下文中的默认模式。',
    'Publish through the current environment chain rooted by located_at, then optionally continue upward when Publish Up is enabled.': '沿由 located_at 锚定的当前环境链传播；启用 Publish Up 后，可以在该环境作用域之外继续向上扩展。',
    'Publish through organization/control links such as belongs_to and subordinate, then optionally continue upward when Publish Up is enabled.': '沿 belongs_to、subordinate 等组织或控制链传播；启用 Publish Up 后，可以在该组织作用域之外继续向上扩展。',
    ...
    'Time': '时间',
    'This action inserts a new primary parent node and rewires only parent_id. It does not create a relation row.': '这个操作会插入一个新的主父节点，并只重连 parent_id，不会额外创建 relation 记录。',
    'Tone': '叙事语气',
    ...
    'Future Outline': '未来大纲',
    'requested_ticks is required for flexible mode and must stay 1 in fixed mode.': '在 flexible 模式下必须填写 requested_ticks，而 fixed 模式下它必须保持为 1。',
    'Requested Ticks': '请求推进 Tick 数',
    'Requested ticks must be greater than 0': '请求推进 Tick 数必须大于 0',
    'Fixed tick scale mode only allows requested_ticks = 1': '固定 Tick 尺度模式下只允许 requested_ticks = 1',
    ...
    'This node already has another located_at relation. Update the existing environment edge instead of stacking multiple active locations.': '这个节点已经有另一条 located_at 关系。请更新现有环境边，而不是叠加多个同时生效的位置。',
