...
| 已完成 | pull 基础 API | 已提供 `pending / claim / start / heartbeat / release / requeue` 六个接口 |
...
| 部分完成 | `push` 出站投递到游戏端 | 已完成 `http_adapter`，`websocket_adapter` / `rpc_adapter` 仍未完成 |
...
| `POST /api/v1/runtime/tasks/requeue` | 已完成 |
| pull 基础测试 | 已完成 |
