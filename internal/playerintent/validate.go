...
func validateStep(view *workerstate.StateView, intent *sdk.PlayerIntent, step sdk.PlayerIntentStep, index int) []ValidationIssue {
	...
	switch stepType {
	case sdk.PlayerIntentTypeShowItem, sdk.PlayerIntentTypeGift, sdk.PlayerIntentTypeUseItem:
		if strings.TrimSpace(step.ItemID) == "" {
			...
			issues = append(issues, ValidationIssue{StepIndex: index, Code: "item_missing", Message: fmt.Sprintf("actor %s does not possess item %s", actorID, step.ItemID), MissingFact: &sdk.MissingFact{Type: sdk.MissingFactItemPresence, NodeID: actorID, ItemID: step.ItemID, Reason: "actor inventory does not contain required item"}})
		}
	case sdk.PlayerIntentTypeSpeech, sdk.PlayerIntentTypeTradeRequest, sdk.PlayerIntentTypeThreaten:
	...
	if sceneID != "" && targetID != "" {
		if targetScene, ok := view.ActorLocation(targetID); !ok || strings.TrimSpace(targetScene) != sceneID {
			issues = append(issues, ValidationIssue{StepIndex: index, Code: "target_not_in_scene", Message: fmt.Sprintf("target %s is not in scene %s", targetID, sceneID), MissingFact: &sdk.MissingFact{Type: sdk.MissingFactTargetLocation, NodeID: targetID, Reason: "target not present in required scene"}})
		}
	}
	return issues
}
...
func validatePrecondition(view *workerstate.StateView, intent *sdk.PlayerIntent, step sdk.PlayerIntentStep, pre sdk.PlayerIntentPrecondition, index int) (ValidationIssue, bool) {
	...
	case sdk.PlayerIntentPreconditionSameScene:
		...
		if !actorOK || !targetOK || actorScene == "" || targetScene == "" || actorScene != targetScene {
			return ValidationIssue{StepIndex: index, Code: "same_scene_failed", Message: fmt.Sprintf("actor %s and target %s are not in the same scene", actorID, targetID), MissingFact: &sdk.MissingFact{Type: sdk.MissingFactTargetLocation, NodeID: targetID, Reason: "same_scene precondition failed"}}, true
		}
	case sdk.PlayerIntentPreconditionTargetPresent:
		if sceneID == "" {
		...
		if targetScene, ok := view.ActorLocation(targetID); !ok || strings.TrimSpace(targetScene) != strings.TrimSpace(sceneID) {
			return ValidationIssue{StepIndex: index, Code: "target_present_failed", Message: fmt.Sprintf("target %s is not present in scene %s", targetID, sceneID), MissingFact: &sdk.MissingFact{Type: sdk.MissingFactTargetLocation, NodeID: targetID, Reason: "target_present precondition failed"}}, true
		}
	case sdk.PlayerIntentPreconditionItemPresent:
		if !view.ItemPresentOnActor(actorID, pre.ItemID) {
			return ValidationIssue{StepIndex: index, Code: "item_present_failed", Message: fmt.Sprintf("actor %s does not have item %s", actorID, pre.ItemID), MissingFact: &sdk.MissingFact{Type: sdk.MissingFactItemPresence, NodeID: actorID, ItemID: pre.ItemID, Reason: "item_present precondition failed"}}, true
		}
	case sdk.PlayerIntentPreconditionMoneyAtLeast:
		money, ok := view.ActorMoney(actorID)
		minimum := intArg(pre.Args, "amount")
		if !ok || money < minimum {
			return ValidationIssue{StepIndex: index, Code: "money_at_least_failed", Message: fmt.Sprintf("actor %s money %d below required %d", actorID, money, minimum), MissingFact: &sdk.MissingFact{Type: sdk.MissingFactWalletState, NodeID: actorID, Reason: "money_at_least precondition failed"}}, true
		}
	case sdk.PlayerIntentPreconditionTaskStatus:
		status, _, ok := view.QuestStatus(pre.TaskID)
		if !ok || !strings.EqualFold(strings.TrimSpace(status), strings.TrimSpace(pre.Expected)) {
			return ValidationIssue{StepIndex: index, Code: "task_status_failed", Message: fmt.Sprintf("task %s status mismatch", pre.TaskID), MissingFact: &sdk.MissingFact{Type: sdk.MissingFactTaskState, TaskID: pre.TaskID, Reason: "task_status precondition failed"}}, true
		}
	case sdk.PlayerIntentPreconditionSceneFlag:
		...
		if !ok || scene == nil || !matchesFlag(scene.Flags, flagName, expected) {
			return ValidationIssue{StepIndex: index, Code: "scene_flag_failed", Message: fmt.Sprintf("scene %s flag %s mismatch", sceneID, flagName), MissingFact: &sdk.MissingFact{Type: sdk.MissingFactSceneState, NodeID: sceneID, Reason: "scene_flag precondition failed"}}, true
		}
	case sdk.PlayerIntentPreconditionLocationAccessible:
		if sceneID == "" {
			return ValidationIssue{StepIndex: index, Code: "location_accessible_failed", Message: "scene_node_id required for location_accessible", MissingFact: &sdk.MissingFact{Type: sdk.MissingFactPlayerLocation, Reason: "scene id required"}}, true
		}
		if _, ok := view.Scene(sceneID); !ok {
			return ValidationIssue{StepIndex: index, Code: "location_accessible_failed", Message: fmt.Sprintf("scene %s not found", sceneID), MissingFact: &sdk.MissingFact{Type: sdk.MissingFactSceneState, NodeID: sceneID, Reason: "scene not found"}}, true
		}
	}
