...
func NewRouter(p *engine.Pipeline) *http.ServeMux {
	...
	mux.HandleFunc("PUT /api/v1/worlds/{world_id}", WorldAccessMiddleware(UpdateWorldHandler))
	mux.HandleFunc("POST /api/v1/worlds/{world_id}/fork", WorldAccessMiddleware(IdempotencyMiddleware(MakeForkWorldHandler(p))))
	...
	mux.HandleFunc("DELETE /api/v1/worlds/{world_id}/snapshot", WorldAccessMiddleware(MakeDeleteWorldSnapshotHandler(p)))
	...
	mux.HandleFunc("POST /api/v1/worlds/{world_id}/restore", WorldAccessMiddleware(IdempotencyMiddleware(MakeRestoreWorldHandler(p))))
	mux.HandleFunc("POST /api/v1/worlds/{world_id}/cold-start", WorldAccessMiddleware(MakeColdStartHandler(p)))
