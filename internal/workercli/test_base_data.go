...
func (a *app) runBaseDataScenario() error {
	result, err := a.executeBaseDataScenario()
	if err != nil {
		return err
	}
...
func (a *app) executeBaseDataScenario() (*baseDataResult, error) {
	if strings.TrimSpace(a.cfg.TestDevCLIExePath) == "" {
		return nil, fmt.Errorf("base-data requires --devcli-exe")
	}
	testsDir := strings.TrimSpace(a.cfg.TestsDir)
	if testsDir == "" {
		return nil, fmt.Errorf("base-data requires --tests-dir")
	}
	fixturePath := filepath.Join(testsDir, "full_functional_base_data_world.yaml")
	if _, err := os.Stat(fixturePath); err != nil {
		return nil, fmt.Errorf("base-data fixture not found: %w", err)
	}
